...
class AV_SPs:
    ...
    def init_compact_arrays(self):
        ...
        for i in range(NDISPARRAY * NENTRY_COMPACT_DISP):
            a = np.array([])
            arraylist.append(a)
        precursors = np.array([], dtype=int)
        arraylist.append(precursors)
    ...
    def append_SP2compact_arrays(self, thisSP, arraylist):
        ...
        arraylist[27] = np.append(arraylist[27], [thisSP.precursor])
        arraylist[28] = np.append(arraylist[28], [thisSP.isconnect])
        return arraylist
    ...
    def valid_SPs(self, arraylist, reasons, Delete=True, insituGSPS=False):
        if Delete:
            if insituGSPS:
                CheckConnectivity = self.sett["ValidSPs"]["CheckConnectivity4insituGSPS"]
            else:
                CheckConnectivity = self.sett["ValidSPs"]["CheckConnectivity"]
            if CheckConnectivity or self.sett["ValidSPs"]["NScreenDisp"] > 0 or self.sett["ValidSPs"]["NScreenEng"] > 0:
    ...
    def validate_SPs(self, Delete=False, insituGSPS=False):
        self.nvalid = self.nSP
        arraylist = self.init_compact_arrays()
        reasons = []
        arraylist, reasons = self.valid_SPs(arraylist, reasons, Delete=Delete, insituGSPS=insituGSPS)
    ...
    def realtime_valid_thisSP(self, thisSP, insituGSPS=False):
        reason = None
        Delete_this = False
        if insituGSPS:
            CheckConnectivity = self.sett["ValidSPs"]["CheckConnectivity4insituGSPS"]
    ...
    def realtime_validate_thisSP(self, thisSP, insituGSPS=False):
        df_delete_this = pd.DataFrame(columns=SP_COMPACT_HEADER4Delete)
        arraylist = self.init_compact_arrays()
        reason, Delete = self.realtime_valid_thisSP(thisSP, insituGSPS=insituGSPS)
        if Delete:
