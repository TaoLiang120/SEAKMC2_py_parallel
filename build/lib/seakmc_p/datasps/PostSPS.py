...
def SPs_1postprocessing(thissett, thisSPS, df_delete_SPs, DFWriter, nSPstart=0, insituGSPS=False):
    ...
    if thissett.saddle_point["ValidSPs"]["RealtimeDelete"]:
        ...
        df_delete_this = thisSPS.validate_SPs(Delete=True, insituGSPS=insituGSPS)
        df_delete_SPs = preSPS.update_df_delete_SPs(df_delete_SPs, df_delete_this, DFWriter)
...
def insert_AVSP2DataSPs(DataSPs, thisSPS, idav, DFWriter, Precursor=False):
    idstart = DataSPs.nSP
    DataSPs, thisdf_SPs = DataSPs.insert_AVSPs(thisSPS, idav)
    if not Precursor: DFWriter.write_SPs(thisdf_SPs, idstart=idstart, mode="a")
    return DataSPs
