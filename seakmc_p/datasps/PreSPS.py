...
def initialize_thisAV(seakmcdata, idav, Rebuild=True, Precursor=False):
    thisAV = seakmcdata.get_active_volume(idav, Rebuild=Rebuild, Precursor=Precursor)
    return thisAV
...
def initial_SNC_CalPref(idav, thisAV, thissett, Precursor=False, insituGSPS=False):
    SNC = thissett.dynamic_matrix["SNC"]
    CalPref = thissett.dynamic_matrix["CalPrefactor"]
    errorlog = ""
    if Precursor: CalPref = False
    if insituGSPS: SNC = False
...
def get_Pre_Disps_from_precursor(idav, thisAV, thissett, seakmcdata, AVitag, thisDataSPs, precursoritags):
    thisitag = AVitag[0:thisAV.nactive]
    Pre_Disps = []
    isRecycled = True
    #print(f"DatanSP: {thisDataSPs.nSP} nP:{len(thisDataSPs.df_SPs)}")
    barrs = []
    for iav in range(len(thisDataSPs.df_SPs)):
        nSP = len(thisDataSPs.df_SPs[iav])
        idpre = thisDataSPs.df_SPs[iav].at[0, "idav"]
        ispstart = thisDataSPs.ispstart[iav]
        thisprecursoritag = precursoritags[idpre]
        napre = thisDataSPs.disps[ispstart].shape[1]
        ijcom, icomind, jcomind = np.intersect1d(thisitag, thisprecursoritag[0:napre], return_indices=True)
        #print(f"iav:{iav} idpre:{idpre} ispstart:{ispstart} nSPlocal:{nSP}")
        #print(f"napre:{napre} ijcom:{ijcom.shape[0]} icomind:{icomind.shape[0]} jcomind:{jcomind.shape[0]}")
        for i in range(nSP):
            thispre = np.zeros([3, thisAV.nactive])
            thisid = ispstart + i
            thisdisps = thisDataSPs.disps[ispstart + i]
            if icomind.shape[0] != napre:
                warningmsg = f"Warning: Inconsistent precursor and AV setups - AV id: {idav} and precursor id: {idpre}!"
                warnings.warn(warningmsg)
            else:
                thispre[:, icomind] = thisdisps[:, jcomind]
                Pre_Disps.append(thispre * thissett.spsearch["Ratio4DispLoad4insituGSPS"])
                barrs.append(thisDataSPs.df_SPs[iav].at[i, "barrier"])
    thisdisps = None
    return isRecycled, Pre_Disps, np.array(barrs)
