...
class SPSearch:
    ...
    def get_scaling_masses(self):
        ...
        mean = np.mean(masses)
        self.Scaling4Masses = masses[types - 1]
        self.Scaling4Masses = np.vstack((self.Scaling4Masses, self.Scaling4Masses, self.Scaling4Masses))
        self.Scaling4Masses = mean / self.Scaling4Masses
...
class Dimer(SPSearch):
    ...
    def dimer_init(self, thisVN=None):
        ...
        self.Rescale = None
        if self.insituGSPS:
            self.isRescale = self.sett.spsearch["HandleVN"]["RescaleVN4insituGSPS"]
            self.RescaleValue = self.sett.spsearch["HandleVN"]["RescaleValue4insituGSPS"].upper()
            self.Ratio4Zero4RAS = self.sett.spsearch["HandleVN"]["Ratio4Zero4RAS4insituGSPS"]
            self.Ratio4Zero4LOGV = self.sett.spsearch["HandleVN"]["Ratio4Zero4LOGV4insituGSPS"]
            self.CenterVN = self.sett.spsearch["HandleVN"]["CenterVN4insituGSPS"]
        else:
    ...
    def update_translation_step(self, itstep):
        ...
        if self.DECAYSTYLE[0:3].upper() == "FIX":
            ...
            if self.insituGSPS:
                self.TSTEPF = (
                            self.sett.spsearch["TrialStepsize4insituGSPS"] * drate + self.sett.spsearch["MinStepsize"])
    ...
    def add_random_dimer(self, itstep):
        def get_decayrate(itstep):
            if self.sett.spsearch["DecayStyle4ARD"][0:3].upper() == "STA":
                return pow(self.sett.spsearch["DecayRate4ARD"], int(itstep / self.sett.spsearch["DecaySteps4ARD"]))
            else:
                return pow(self.sett.spsearch["DecayRate4ARD"],
                           float(itstep) / float(self.sett.spsearch["DecaySteps4ARD"]))

        VN = np.random.rand(3, self.nactive) - 0.5
        drate = self.sett.spsearch["BaseRatio4ARD"] * get_decayrate(itstep)
        self.VN += VN * drate
                ...
                if self.FNUNIT < self.sett.spsearch["FMin4Rot"]:
                    ...
                    if self.RO_FWD:
                        if self.NROITR > self.sett.spsearch["NMax4Rot"]:
                            ...
                            self.RO_OPT = True
                        #if self.AddRandomDirection and self.RO_OPT and self.ROCURV<0.0:
                        if self.AddRandomDirection and self.RO_OPT:
                            self.add_random_dimer(self.NTSITR)
                            if self.FixType: self.VN = self.VN * self.Scaling4Types
                            self.VN = mat_unit(self.VN)
                            self.XC = self.X0 + np.multiply(self.VN, self.RDIMER)
                        self.dimer_projection(TR_HOR=self.TR_HOR)
    ...
    def dimer_search(self, thisSPS, ReSearch=False):
        ...
        while not self.DI_CON:
            ...
            if self.ST_CON and self.sett.spsearch["CheckAng"] and self.NTSITR % self.sett.spsearch[
                "CheckAngSteps"] == 0:
            ...
            if self.NTSITR >= self.NMax4Trans:
                self.DI_MAX = True
                self.DI_CON = True
    ...
    def dimer_re_search(self, thisSPS, nactive=None):
        ...
        if ndiff <= 0:
            warnings.warn(
                "The number of atoms for this dimer search is smaller or equal than number of atoms of last one.")
            isValid = False
        if nfixed < 0:
            warnings.warn("The number of atoms for dimer search is larger than number of atoms in active volume.")
            isValid = False

        if isValid and self.DI_MAX == False:
