...
class SuperBasin:
    ...
    def insert_Basin(self, thisBasin):
        ...
        self.meanpref = self.meanpref / self.nSP
        #print(f"meanpref:{self.meanpref} nsp:{self.nSP}")
        #print("===")
            ...
            for ib in range(self.nBasin):
                if isVisit:
                    ...
                    if iba == ib:
                        ...
                        maxdiff = np.sqrt(np.max(diffs))
                        if maxdiff <= Tol4Disp:
                            self.remove_backward_SP(bas, Tol4Disp=Tol4Disp, Tol4Barr=Tol4Barr,
                                                    ForwardRate=self.frate, Handle_no_Backward="STAYIN")
                            self.now = bas.id
                            isVisit = True
            return isVisit
