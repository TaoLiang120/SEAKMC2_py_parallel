...
SP_DATA_HEADER = ["idav", "idsps", "barrier", "prefactor", "ebias", "dmag", "dmax", "dsum", "adsum", "dmagfi", "dmaxfi",
                  "dsumfi", "adsumfi", "isConnect"]
SP_KMC_HEADER = ["idav", "idsps", "barrier", "prefactor", "ebias", "dmag", "dmax", "dsum", "dsumr", "dmagfi", "dmaxfi",
                 "dsumfi", "dsumrfi", "isConnect", "probability", "isSel"]
...
class Settings:
    ...
    def from_file(cls, filename):
        ...
        if "atom_style" not in data:
            raise ValueError("There must be an atom_style for data!")
        ...
        if thisfeval["TrialDisps2Basin"]["TrialDisps2Basin"]:
            if not isinstance(thisfeval["TrialDisps2Basin"]["Rinput4TDB"], str):
                logstr = "There must be a FileName for applying displacements to the basin!"
                error_exit(logstr)
            if thisfeval["TrialDisps2Basin"]["Ref_Length"] is None:
                logstr = "There must be a reference length for applying displacements to the basin!"
                error_exit(logstr)
            if thisfeval["TrialDisps2Basin"]["Target_StrainRate"] is None:
                logstr = "There must be a target strain rate for applying displacements to the basin!"
                error_exit(logstr)
        ...
        if not "dangercut" in potential: potential['dangercut'] = 0.5 * min(bondlengths.flatten())
        ...
        HandleVN = {"CheckAng4Init": True, "AngTol4Init": 5.0, "MaxIter4Init": 20, "NMaxRandVN": 20,
                    ...
                    "RescaleVN": True, "RescaleValue": "LOGN", "AppRescaleRot": True, "Int4ComputeScale": 1,
                    ...
                    "RescaleVN4insituGSPS": True, "RescaleValue4insituGSPS": "LOGN", "CenterVN4insituGSPS": False,
                    "Ratio4Zero4RAS4insituGSPS": 0.3, "Ratio4Zero4LOGV4insituGSPS": 0.2}
        thisspsearch = {"Method": "dimer", "NSearch": 10, "SearchBuffer": False, "NMax4Trans": 1000, "IgnoreSteps": 4,
                        ...
                        "AddRandomDirection": False, "BaseRatio4ARD": 0.2, "DecayRate4ARD": 0.71, "DecaySteps4ARD": 10,
                        "DecayStyle4ARD": "Contineous",
                        "Tol4Connect": 0.1, "FixTypes": False, "FixAxesStr": "ALL",
                        "insituGuidedSPS": False, "TrialStepsize4insituGSPS": 0.015, "MaxStepsize4insituGSPS": 0.05,
                        "Ratio4DispLoad4insituGSPS": 0.9,
                        "TransHorizon4insituGSPS": True, "NMax4Trans4insituGSPS": 1000, "NMaxSPs4insituGSPS": "NA",
        ...
        Write_AV_SPs = {"Write_Local_AV": False, "Write_AV_SPs": False, "Write_Data_AV_SPs": False,
                        "AVOutPath": "AVOut", "DispStyle4AVSP": "Both", "OutputStyle": "SEP"}
    ...
    def validate_input(self):
        ...
        if self.spsearch["insituGuidedSPS"]:
            ...
            self.saddle_point["ValidSPs"]["RealtimeDelete"] = True
            self.saddle_point["ValidSPs"]["RealtimeValid"] = False
        ...
        if self.spsearch["MaxStepsize"] <= self.spsearch["TrialStepsize"]:
            ...
            error_exit(logstr)
        if self.spsearch["MaxStepsize4insituGSPS"] <= self.spsearch["TrialStepsize4insituGSPS"]:
            logstr = "The MaxStepsize4insituGSPS must be larger than TrialStepsize4insituGSPS!"
            error_exit(logstr)
                        ...
                        if isinstance(fixtype, int):
                            ...
                            for j in range(len(thisstr)):
                                try:
                                    ...
                                    axis = -1
                                    errormsg += ("\n" +
                                                 "FixAxesStr must be a string with axis index numbers separating with commas.")
                                    isValid = False
                            ...
                            errormsg += ("\n" +
                                         "FixTypes must be an integer ranging from 1 to the number of types of the data.")
                            isValid = False
                            break
            else:
                a = [0, 1, 2]
